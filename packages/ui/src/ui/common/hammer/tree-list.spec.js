...
describe('hammer.tree-list', () => {
    ...
    describe('API', () => {
        ...
        describe('filterTree()', () => {
            describe('behavior', () => {
                let preparedTreeNodes, filteredNodes;

                beforeEach(() => {
                    preparedTreeNodes = treeList.prepareTree(entries, parentGetter);
                });

                it('Does not change tree structure', () => {
                    filteredNodes = treeList.filterTree(preparedTreeNodes[ROOT_NODE], () => true);
                    expect(filteredNodes.name).toBe(preparedTreeNodes[ROOT_NODE].name);
                    expect(filteredNodes.children.length).toBe(
                        preparedTreeNodes[ROOT_NODE].children.length,
                    );
                    expect(filteredNodes.leaves.length).toBe(
                        preparedTreeNodes[ROOT_NODE].leaves.length,
                    );
                });

                it('Removes non matching children and leaves', () => {
                    filteredNodes = treeList.filterTree(
                        preparedTreeNodes[ROOT_NODE],
