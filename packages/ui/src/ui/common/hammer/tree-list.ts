...
export function filterTreeEachChild<T extends TreeNode<unknown, unknown>>(
    ...
    const newChildren = treeNode.children?.reduce(
        (acc, item) => {
            const filtered = filterTreeEachChild(item as T, nodePredicate, leafPredicate);
            if (filtered) {
                acc.push(filtered);
            }
            return acc;
        },
        [] as typeof treeNode.children,
    );

    const newLeaves = leafPredicate
