...
export type YTChartKitHistogramProps = {
    ...
    /**
     * Fixed width of a bar, the bars are aligned to zero, so the bars of different charts of the
     * same metric are comparable.
...
export function YTChartKitHistogram(props: YTChartKitHistogramProps) {
    ...
    const chartData = React.useMemo(() => {
        ...
        let step = (effectiveMax - from) / maxBarCount;
        let effectiveBarCount = maxBarCount;
        if (barWidth !== undefined) {
            ...
            effectiveBarCount = Math.max(1, Math.ceil((effectiveMax - from) / step));
        }

        let maxSum = 0;
        const values = data.reduce(
            (acc, v) => {
                // The epsilon keeps round values on their bar border, e.g. 0.3 / 0.1 is 2.9999...
                // The last bar is half-open, rounding of |effectiveMax| may move a value out of it.
                const index = Math.min(effectiveBarCount - 1, Math.floor((v - from) / step + 1e-9));
                acc[index] += 1;
                maxSum = Math.max(maxSum, acc[index]);
                return acc;
            },
            Array.from({length: effectiveBarCount}, () => 0),
        );

        const to = from + step * effectiveBarCount;
        ...
        const res: ChartData = {
            ...
            tooltip: {
                renderer({hovered}) {
                    ...
                    const {y, index} = barValues as typeof barValues & {index: number};
        ...
        return res;
    }, [data, xPlotLines, barWidth, maxBarCount, seriesName, yAxisMin, xAxisTitle, yAxisTitle]);
