...
class MapNodesTable extends Component {
    ...
    static renderTrash(linkState) {
        return <FormattedLink text={i18n('action_trash')} state={linkState} theme="primary" />;
    }
    ...
    static renderNameImpl(item) {
        ...
        if (type === 'scheduler_pool') {
            ...
            const goToLink = (
                <Link url={url} title={url}>
                    <span>{i18n('action_go-to-scheduling')}</span>
                </Link>
            );
        ...
        if (type === 'link') {
            ...
            const viewLink = (
                <FormattedLink
                    text={i18n('action_view-link')}
                    state={item.linkPathState}
                    theme="ghost"
                />
            );
    ...
    static renderLocks(item) {
        return (
            item.locks > 0 && (
                <span title={i18n('context_locks', {count: item.locks})}>
    ...
    onItemClick = (item, _index, evt) => {
        if (evt?.target?.closest?.('.map-node_default__table-item_type_chooser')) {
