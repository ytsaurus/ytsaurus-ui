...
export const useDragAndDrop = <Formats extends FileFormats>({
    availableFormats,
    progress,
    onFileChange,
    onError,
    ...
    const checkFile = (file: File | null): string | null => {
        ...
        if (!availableFormats.find((format) => FORMATS[format].type === type)) {
            return `Invalid file format. Instead of ${type} it should be ${
                availableFormats.length > 1 ? `one of these: ${availableFormats}` : availableFormats
            } `;
        }
        ...
        if (file) {
            ...
            const fileType =
                (Object.keys(FORMATS).find(
                    (format) => FORMATS[format as Formats].type === file.type,
                ) as Formats) ?? null;
