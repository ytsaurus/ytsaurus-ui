        ...
        parts.forEach(({color, percentage, state, phases, interval}) => {
            ...
            if (state.toLowerCase() === 'running') {
                ...
                for (let i = 0; i < phaseCount; i++) {
                    ...
                    const lineColor = isRunningPhase ? color : getColor('--purple-color');

                    ctx.beginPath();
                    ctx.fillStyle = convertToRGBA(lineColor || '#333', isTransparent ? 0.4 : 0.7);
                    ctx.rect(
                        startPosition,
                        y0,
                ...
                ctx.beginPath();
                ctx.fillStyle = convertToRGBA(color || '#333', isTransparent ? 0.4 : 0.7);
