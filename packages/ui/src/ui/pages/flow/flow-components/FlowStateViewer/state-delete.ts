...
export async function runDeleteAttempt({
    session,
    rows,
    force,
    refreshPermission,
    ...
    force: boolean;
    refreshPermission: () => Promise<{action: 'allow' | 'deny'} | undefined>;
    refreshPipelineState: () => Promise<GetPipelineStateData>;
    execute: (body: FlowDeleteStatesBody) => Promise<FlowDeleteStatesResponse>;
    isSessionCurrent: (session: number) => boolean;
}): Promise<FlowDeleteStatesRunResult> {
    let freshPermission;
    ...
    const outcomes = await runRowDeletes(rows, execute, {
        force,
        isCancelled: () => !isSessionCurrent(session),
    });
    return isSessionCurrent(session)
...
export function getRowsPendingDelete(
    rows: Array<FlowStateResultRow>,
    ...
    return rows.filter((row) => !committedRowIds.has(getStateRowId(row)));
}

export function mergeDeleteOutcomes(
    rows: Array<FlowStateResultRow>,
