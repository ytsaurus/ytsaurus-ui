...
const RAW_KEY_TOKEN_DECODERS: Record<string, RawKeyTokenDecoder> = {
    int64: decodeSignedInteger,
    int32: decodeSignedInteger,
    int16: decodeSignedInteger,
    int8: decodeSignedInteger,
    uint64: decodeUnsignedInteger,
    uint32: decodeUnsignedInteger,
    uint16: decodeUnsignedInteger,
    uint8: decodeUnsignedInteger,
    double: decodeFiniteNumber,
    float: decodeFiniteNumber,
...
function decodeRawKeyToken(token: string, column: FlowKeyColumn): string | undefined {
    ...
    return RAW_KEY_TOKEN_DECODERS[column.type]?.(trimmed);
}
