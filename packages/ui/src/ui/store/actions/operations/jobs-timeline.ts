            ...
            const result = response.reduce<Pick<JobsTimelineState, 'events' | 'eventsInterval'>>(
                (acc, {output: job}) => {
                    ...
                    for (let i = 0; i < eventsCount; i++) {
                        ...
                        if (!(job.job_id in acc.events)) {
                            acc.events[job.job_id] = [];
                        }

                        const startTime = dayjs(event.time).valueOf();
                        const prevEvent = acc.events[job.job_id][stateIndex];
                        if (event.state) {
                            acc.events[job.job_id].push({
                                state: event.state,
                                startTime,
                                endTime: 0,
                                phases: [],
                            ...
                            stateIndex = acc.events[job.job_id].length - 1;
                        } else {
                            acc.events[job.job_id][stateIndex].phases.push({
                                phase: event.phase!,
                                startTime,
                            });
                        }
