...
export function setQueryEngineFilter(
    id: string,
    engine: string,
    ...
    return (dispatch, getState) => {
        ...
        const newItem = {...oldItem, data: {...oldItem?.data, engine}};
...
export function setQueryStateFilter(
    id: string,
    queryState: string,
): ThunkAction<any, RootState, any, any> {
    return (dispatch, getState) => {
        ...
        const newItem = {...oldItem, data: {...oldItem?.data, state: queryState}};
        const newItems = [...filter_(config.items, (item) => !isEqual_(item, oldItem)), newItem];
        const newConfig = {...config, items: newItems};
