...
jest.mock('../utils', () => ({
    useEffectiveClusterArgs: (args: unknown) => args,
}));
...
describe('flowReadStates endpoint', () => {
    it('sends typed formats and normalizes the annotated response', async () => {
            ...
            key_states: [
                {
                    ...
                    key: [{$type: 'uint64', $value: '5'}, 'ÐÐµÐ½Ð¸Ðº-Ñ-Ð¿ÑÐ¾Ð²Ð¾Ð»Ð¾ÐºÐ¾Ð¹'],
        ...
        expect(args.setup.JSONSerializer).toBeDefined();
        expect(args.data).toEqual({computation_id: 'c1', limit: 10});
        expect(result.data).toEqual({
            key_states: [{computation_id: 'c1', key: [5, 'Веник-с-проволокой'], states: {'/s': 7}}],
        });
    });
});
