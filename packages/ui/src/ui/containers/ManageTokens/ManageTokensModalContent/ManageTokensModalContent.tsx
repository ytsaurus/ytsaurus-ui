...
const ManageTokenPasswordGuard = () => {
    const {getPassword} = useManageTokensPasswordModalContext();
    const dispatch = useDispatch();
    const [statePasswordSha256, setStatePasswordSha256] = React.useState<string | undefined>();
    const [open, setOpen] = React.useState(false);

    React.useEffect(() => {
        getPassword()
            .then((passwordSha256) => {
                setStatePasswordSha256(passwordSha256);
                setOpen(true);
            })
            .catch(() => {
    ...
    return <ManageTokensModalSections passwordSha256={statePasswordSha256} />;
};
