...
export function updatePoolChild(data, cypressData, type, treeResources, opts) {
    ...
    if (data.type === 'pool') {
        if (typeof attributes === 'undefined' && data.parent) {
            console.error('Pool "%s" without attributes inited by "%s"', data.name, data._initedBy);
        }
        ...
        if (!data.leaves?.length) {
            data.pool_operation_count = ypath.getNumber(attributes, '/pool_operation_count', NaN);
        ...
        data.maxOperationCountEdited = ypath.getNumber(cypressAttributes, '/max_operation_count');
        data.runningOperationCount = ypath.getNumber(attributes, '/running_operation_count');
        data.maxRunningOperationCount = ypath.getNumber(attributes, '/max_running_operation_count');
    ...
    data.flowCPU = ypath.getNumber(attributes, '/specified_resource_flow/cpu', userDefinedFlowCPU);
    const userDefinedFlowGPU = ypath.getNumber(
        cypressAttributes,
        '/integral_guarantees/resource_flow/gpu',
        NaN,
    );
    data.flowGPU = ypath.getNumber(attributes, '/specified_resource_flow/gpu', userDefinedFlowGPU);
    ...
    data.fifoSortParams =
        fifoSortParams.length > 0 ? fifoSortParams : ['start_time', 'weight', 'pending_job_count'];
