...
import {AIChatConfig, type Conversation, GetConversationItemsResponse} from '../../shared/ai-chat';
import {
...
export class AiChat {
    ...
    async createConversation(data: CreateConversationRequest, req: Request): Promise<Conversation> {
    ...
    async deleteConversation(conversationId: string, req: Request): Promise<void> {
        await axios.request({
    ...
    async summarizeConversationTitle(conversationId: string, req: Request): Promise<string> {
        const {data} = await this.getConversationItems(
            conversationId,
